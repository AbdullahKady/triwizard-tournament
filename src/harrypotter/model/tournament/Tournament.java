...
public class Tournament implements TaskListener {
	...
	private void loadSpells(String filePath) throws IOException {
		...
		while (line != null) {
			...
			case "DMG":
				spells.add(new DamagingSpell(content[1], Integer.parseInt(content[2]), Integer.parseInt(content[4]),
						Integer.parseInt(content[3])));
				break;

			case "HEL":
				spells.add(new HealingSpell(content[1], Integer.parseInt(content[2]), Integer.parseInt(content[4]),
						Integer.parseInt(content[3])));
				break;

			case "REL":
				spells.add(new RelocatingSpell(content[1], Integer.parseInt(content[2]), Integer.parseInt(content[4]),
						Integer.parseInt(content[3])));
				break;
	...
	private Wizard winnerFinal;

	public void endGame() {
		if (this.endGame) {
			JOptionPane.showMessageDialog(null, winnerFinal.getName() + " HAS WON !!!!", "HINT",
					JOptionPane.INFORMATION_MESSAGE);
			JOptionPane.showMessageDialog(null, "THANKS FOR PLAYING OUR GAME <3", "HINT",
					JOptionPane.INFORMATION_MESSAGE);
			System.exit(0);
