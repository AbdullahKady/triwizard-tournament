...
public abstract class Task implements WizardListener {
	...
	public void moveForward() throws IOException, OutOfBordersException, InvalidTargetCellException {
		...
		if (getMap()[newLocation.x][newLocation.y] instanceof ChampionCell
			...
			throw new InvalidTargetCellException(
					"Cannot move to " + getMap()[newLocation.x][newLocation.y].getClass().getSimpleName());

		}

		if (getMap()[newLocation.x][newLocation.y] instanceof CollectibleCell)
			current.getInventory().add(((CollectibleCell) getMap()[newLocation.x][newLocation.y]).getCollectible());
	...
	public void castDamagingSpell(DamagingSpell spell, Direction d) throws IOException, OutOfBordersException,
			InvalidTargetCellException, InCooldownException, NotEnoughIPException {
		...
		if (spell.getCost() > ((Wizard) currentChamp).getIp()) {
			throw new NotEnoughIPException(spell.getCost(), spell.getCost() - ((Wizard) currentChamp).getIp());
		}

		Point target = getTargetPoint(d);

		if (target.x < 0 || target.x > 9 || target.y < 0 || target.y > 9) {
			throw new OutOfBordersException("Cannot cast a damaging spell on a target beyond the borders.");
		}

		if (map[target.x][target.y] instanceof ObstacleCell) {

			Obstacle obstacle = ((ObstacleCell) map[target.x][target.y]).getObstacle();
			...
			Wizard opponent = (Wizard) (((ChampionCell) map[target.x][target.y]).getChamp());
			int newHp = 0;

			if (this instanceof ThirdTask && opponent instanceof HufflepuffWizard) {
			...
			throw new InvalidTargetCellException(
					"Cannot cast a damaging spell on " + map[target.x][target.y].getClass().getSimpleName());
	...
	public void castRelocatingSpell(RelocatingSpell spell, Direction d, Direction t, int range)
			throws IOException, OutOfBordersException, InvalidTargetCellException, InCooldownException,
		...
		if (target.x < 0 || target.x > 9 || target.y < 0 || target.y > 9) {
			throw new OutOfBordersException("Cannot cast a relocating spell on a target beyond the borders.");
		}
		...
		if (newX < 0 || newX > 9 || newY < 0 || newY > 9) {
			throw new OutOfBordersException("Cannot cast a relocating spell with a destination beyond the borders.");
		}

		if (!(map[newX][newY] instanceof EmptyCell)) {

			throw new InvalidTargetCellException(
					"Cannot cast a relocating spell with destination " + map[newX][newY].getClass().getSimpleName());
		...
		if (map[target.x][target.y] instanceof ObstacleCell) {
			...
			Champion opponent = ((ChampionCell) map[target.x][target.y]).getChamp();
			...
			throw new InvalidTargetCellException(
					"Cannot cast a relocating spell on target " + map[target.x][target.y].getClass().getSimpleName());
	...
	public void castHealingSpell(HealingSpell spell) throws IOException, InCooldownException, NotEnoughIPException {
	...
	public void useSpell(Spell spell) {
		...
		((Wizard) currentChamp).setIp(((Wizard) currentChamp).getIp() - spell.getCost());
	...
	public void onSlytherinTrait(Direction d)
		...
		if (newX < 0 || newX > 9 || newY < 0 || newY > 9) {
			...
			throw new OutOfBordersException("Cannot use the Slytherin trait to move beyond the borders.");
		...
		if (!(map[newX][newY] instanceof EmptyCell)) {
			...
			throw new InvalidTargetCellException(
					"Cannot use the Slytherin trait to move to " + map[newX][newY].getClass().getSimpleName());
